...
def LinuxProfileFactory(profpkg):
    ...
    class AbstractLinuxProfile(obj.Profile):
        ...
        def load_vtypes(self):
            ...
            debug.debug("{2}: Found dwarf file {0} with {1} symbols".format(
                f.filename, len(vtypesvar.keys()), profilename))
    ...
    for path, _, files in os.walk(path):
        for fn in files:
            if zipfile.is_zipfile(os.path.join(path, fn)):
                new_classes.append(LinuxProfileFactory(
...
class module_sect_attr(obj.CType):
    ...
    def sect_name(self):
        if type(self.m("name")) == obj.Array:
            name = obj.Object("String", offset=self.m(
                "name").obj_offset, vm=self.obj_vm, length=32)
        else:
...
class module_struct(obj.CType):
    ...
    def _get_sect_count(self, grp):
        arr = obj.Object(theType='Array', offset=grp.attrs,
                         vm=self.obj_vm, targetType='Pointer', count=25)
    ...
    def get_sections(self):
        ...
        attrs = obj.Object(theType='Array', offset=self.sect_attrs.attrs.obj_offset,
                           vm=self.obj_vm, targetType='module_sect_attr', count=num_sects)
    ...
    def get_param_val(self, param, _over=0):
        ...
        if getfn == 0:
            ...
            val = str(param.str.dereference_as(
                "String", length=param.str.maxlen))
    ...
    def get_params(self):
        ...
        params = ""
        param_array = obj.Object(theType='Array', offset=self.kp,
                                 vm=self.obj_vm, targetType='kernel_param', count=self.num_kp)

        for param in param_array:
            val = self.get_param_val(param)
            params = params + \
                "{0}={1} ".format(param.name.dereference_as(
                    "String", length=255), val)
    ...
    def get_symbols(self):
        ...
        syms = obj.Object(theType="Array", targetType=struct_name,
                          offset=self.symtab, count=self.num_symtab + 1, vm=self.obj_vm)
...
class task_struct(obj.CType):
    ...
    def get_elf(self, elf_addr):
        ...
        for start in sorted(sects.keys()):
            ...
            if last_end != -1 and last_end != start + read_size:
                debug.error("busted LOAD segments in %s | %d -> %x != %x + %x" %
    ...
    def bash_hash_entries(self):
        ...
        for off in self.search_process_memory(["\x40\x00\x00\x00"], heap_only=True):
            ...
            htable = obj.Object("_bash_hash_table",
                                offset=off - nbuckets_offset, vm=proc_as)
    ...
    def _dynamic_env(self, proc_as, pack_format, addr_sz):
        ...
        for vma in self.get_proc_maps():
            ...
            envars = obj.Object(
                theType="Array", targetType="Pointer", vm=proc_as, offset=env_start, count=256)
            for var in envars:
                if var:
    ...
    def _shell_variables(self, proc_as, pack_format, addr_sz):
        ...
        for vma in self.get_proc_maps():
            ...
            for off in range(0, vma_len - addr_sz, 4):
                ...
                htable = obj.Object("_bash_hash_table",
                                    offset=ptr2, vm=proc_as)

                for ent in htable:
                    key = str(ent.key.dereference())
                    val = str(ent.data.dereference_as(
                        "_envdata").value.dereference())
    ...
    def lsof(self):
        ...
        fds = obj.Object(theType='Array', offset=fds.obj_offset,
                         vm=self.obj_vm, targetType='Pointer', count=max_fds)
                ...
                if inet_sock.protocol in ("TCP", "UDP", "IP", "HOPOPT"):
                    ...
                    if family == 1:  # AF_UNIX
                        unix_sock = obj.Object(
                            "unix_sock", offset=inet_sock.sk.v(), vm=self.obj_vm)

                        if unix_sock.addr:
                            name_obj = obj.Object(
                                "sockaddr_un", offset=unix_sock.addr.name.obj_offset, vm=self.obj_vm)
                ...
                for dsec in phdr.dynamic_sections():
                    ...
                    if dsec.d_tag == 3:  # DT_PLTGOT
                        ...
                        link_map_addr = obj.Object(
                            "Pointer", offset=got_start + (dsec.size_cache / 8), vm=proc_as)
                        link_map = obj.Object(
                            "elf_link_map", offset=link_map_addr, vm=proc_as, parent=dsec)
                        for ent in link_map:
    ...
    def threads(self):
        ...
        threads = [self]
        x = obj.Object('task_struct', self.thread_group.next.v() -
                       thread_offset, self.obj_vm)
        while x not in threads and x.is_valid() and x.thread_group.is_valid() and x.thread_group.next.is_valid():
            threads.append(x)
            x = obj.Object('task_struct', x.thread_group.next.v() -
                           thread_offset, self.obj_vm)
        return threads
    ...
    def elfs(self):
        ...
        for vma in self.get_proc_maps():
            ...
            if pt_loads:
                (elf_start, elf_end) = (
                    min(s[0] for s in pt_loads), max(s[1] for s in pt_loads))
            else:
                continue
    ...
    def get_task_start_time(self):
        ...
        if boot_time != -1:
            ...
            dt = obj.Object("UnixTimeStamp", offset=0,
                            vm=bufferas, is_utc=True)
        else:
            dt = None
...
class VolatilityDTB(obj.VolatilityMagic):
    ...
    def generate_suggestions(self):
        ...
        if profile.metadata.get('memory_model', '32bit') == "32bit":
            ...
            shifts = [0xffffffff80000000, 0xffffffff80000000 -
                      0x1000000, 0xffffffff7fe00000]
            read_sz = 8
            fmt = "<Q"
        ...
        try:
            ...
            idle_class_addr = tbl["idle_sched_class"][0][0] + \
        ...
        if physical_shift_address != 0 and virtual_shift_address != 0:
            good_dtb = (
                dtb_sym_addr - shifts[0] - virtual_shift_address) + physical_shift_address
...
class VolatilityLinuxIntelValidAS(obj.VolatilityMagic):
    ...
    def generate_suggestions(self):
        ...
        for shift in shifts:
            check = init_task_addr - shift + self.obj_vm.profile.physical_shift - \
    ...
    def to_paddr(self):
        ...
        if mem_map_addr:
            ...
            mem_map_ptr = obj.Object(
                "Pointer", offset=mem_map_addr, vm=self.obj_vm, parent=self.obj_parent)
        ...
        phys_offset = (self.obj_offset - mem_map_ptr) / \
...
class vfsmount(obj.CType):
    ...
    def _get_real_mnt(self):
        ...
        mnt = obj.Object("mount", offset=self.obj_offset -
                         offset, vm=self.obj_vm)
        return mnt
